...
_upd() {
   if [[ -d "$_hyprconf" ]]; then
       echo -e ":: hyrconf-v2 dir is available in the cache. Removing it"
       echo
...
choice=$(
        gum confirm \
        "Would you like to update your current 'hyprconf-v2'?" \
...
case $1 in 
    --hyprconf)
        kitty --title update sh -c "$HOME/.config/hypr/scripts/hyprconf-v2.sh"
        ;;
esac
